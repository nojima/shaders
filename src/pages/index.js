...
const newCamera = () => {
  return new THREE.OrthographicCamera(-0.5, 0.5, 0.5, -0.5, 0.0, 1000.0);
};

const newRenderer = (canvasElement, width, height) => {
...
const newShaderMaterial = (vertexShader, fragmentShader, uTime, uResolution) => {
  const uniforms = { uResolution, uTime };
...
const WebGLCanvas = (props) => {
  const {width, height, vertexShader, fragmentShader} = props;
  const canvasElement = React.useRef();

  React.useEffect(() => {
    const scene = newScene();
    const camera = newCamera();
    const renderer = newRenderer(canvasElement, width, height);

    const uTime = new THREE.Uniform(0.0);
    const uResolution = new THREE.Uniform(new THREE.Vector2(width, height));

    const geometry = new THREE.PlaneGeometry();
    const material = newShaderMaterial(vertexShader, fragmentShader, uTime, uResolution);
  ...
  }, [width, height, vertexShader, fragmentShader, canvasElement]);

  return (
    <canvas className="WebGLCanvas" ref={canvasElement}/>
  );
}
